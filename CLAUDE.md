...
- `tools/probar-bom.ps1` prueba el servicio con un fichero de materiales real: lee `.xlsx` sin Excel ni módulos, manda solo la cabecera y unas filas, pide el mapeo de columnas con salida estructurada y enseña lo que costó la llamada. `-SoloMuestra` enseña lo que se enviaría sin gastar crédito. Guía en `docs/planificacion/prueba-bom.md`.
