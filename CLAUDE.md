...
| Subdominio del VPS | `apisor.oracle402.com` (variable `API_DOMINIO`) |
| Puerto interno en el VPS | `8081` (variable `VPS_PUERTO_INTERNO`) |
...
| Backend (Fly.io, opcional) | `https://<app de Fly>.fly.dev/` · `/salud` · `/holamundo` | `.github/workflows/deploy.yml` en push a `main` que toque `app/**`; previsualización de `main` |
| **Backend en el VPS (producción)** | https://apisor.oracle402.com/ · `/salud` · `/v1/...` | `.github/workflows/deploy-vps.yml` en push a `release` que toque `app/**` o `vps/**`; `release` solo avanza cuando el usuario lo pide |
...
| **Conectar una aplicación (Pages)** | https://npiobject-labs.github.io/openrouter/conectar.html | asistente de alta: crea la app, le pone topes, prueba su clave y entrega el código; es la guía para cada nueva conexión |
...
- Secretos y variables del backend: `OPENROUTER_API_KEY` y `SERVICIO_CLAVE` (secretos de repositorio, los vuelca `deploy.yml` a Fly), `MODELO_DEFECTO` (variable de repositorio, opcional), `CORS_ORIGENES` (lista de orígenes separada por comas; vacía = cualquiera; en el VPS la fija `deploy-vps.yml` desde `VPS_CORS_ORIGENES`, por defecto Pages), y `OPENROUTER_BASE` y `BD_RUTA` (solo para pruebas locales).
- **El VPS tiene sus propios secretos**, con prefijo `VPS_`: `VPS_HOST`, `VPS_USUARIO`, `VPS_SSH_CLAVE` (clave dedicada, la crea `tools/vps-clave.ps1`), `VPS_HOST_KEY` (huella del servidor), `VPS_OPENROUTER_API_KEY` (clave de OpenRouter distinta de la de Fly, con límite de crédito en OpenRouter) y `VPS_SERVICIO_CLAVE` (clave de administración del VPS, distinta de la de Fly). Variables: `VPS_PUERTO` (SSH; es variable y no secreto porque como secreto GitHub enmascara cada «22» del log), `VPS_PUERTO_INTERNO`, `API_DOMINIO`. Fly y el VPS son dos despliegues con dos bases y dos juegos de claves de aplicación: no se sincronizan.
- `openrouter-backend --salud` es el healthcheck del contenedor: pide `/salud` al propio proceso y sale con 0 o 1. La imagen no lleva `curl`. El apagado limpio atiende SIGINT y SIGTERM (el de `docker stop` y el de Fly).
- **La verificación de un despliegue es un solo script**, `tools/verificar-servicio.sh <url> <sha>`, que llaman `deploy.yml` y `deploy-vps.yml`: `/salud` con el SHA, `/holamundo` con CORS, el sobre de error, `/v1/apps` sin claves, `/v1/presupuesto` con `administracion:true`, `/v1/uso`, y con clave de OpenRouter `/v1/estado` (`almacen:sqlite`) y `/v1/models`. Si añades una comprobación, va ahí y vale para los dos.
- `/v1/uso`, `/v1/uso/resumen` y `/v1/uso/exportar` aceptan `?operacion=` (el valor de `X-Operacion`), y el resumen `agrupar=operacion`: es como una aplicación de presupuestos pregunta cuánto costó cada presupuesto.
...
- **Toda conexión de una aplicación nueva empieza en `docs/conectar.html`**, y se llega desde la consola (llamada bajo la cabecera y dentro del panel de Aplicaciones) y desde el pie de `consola.html`, `api.html` e `index.html`. Una página a la que solo se llega escribiendo la URL no existe: hace de verdad cada paso (alta, topes, prueba con la clave de la aplicación) y entrega el fragmento de código para PowerShell, Python, JavaScript, Rust y curl con la URL del servidor elegido y la clave como marcador. Comparte servidores y clave de administración guardada con la consola. Su lista de reglas para el repositorio consumidor (secreto `LLM_API_KEY`, módulo aislado, funcionar sin clave, proteger la propia API) es la referencia; `docs/planificacion/integracion-gestionpresupuestos.md` es la misma guía en texto. `tools/probar-clave.ps1` hace el paso de prueba desde el PC.
- **Las cabeceras también son contrato.** El JSON sale con `application/json; charset=utf-8` (PowerShell 5.1 decodifica en Latin-1 lo que no lo declara) y el CORS admite `X-Operacion` en el preflight (sin eso, el navegador bloquea a cualquier cliente web que siga el contrato). `tools/verificar-servicio.sh` comprueba las dos en cada despliegue: las encontró el primer consumidor real, no la verificación.
- `docs/consola.html` es el cliente de referencia del servicio y crece con cada etapa. Tiene selector de servidor (VPS por defecto, Fly, y cualquier otro con `?api=https://...`) y guarda una clave por servidor en `localStorage`. `docs/index.html` es el mock de todas las etapas, no llama a nada.
...
- `deploy.yml` (Fly) y `deploy-vps.yml` (VPS) terminan llamando a `tools/verificar-servicio.sh`, que hace `curl` a `/salud` (falla si la respuesta no contiene el SHA del commit), a `/holamundo`, al sobre de error y, con clave, a `/v1/apps`, `/v1/presupuesto`, `/v1/uso`, `/v1/estado` y `/v1/models`. Las que hablan con OpenRouter solo corren si el despliegue tiene su clave; no gastan crédito.
...
- **Backend de producción: el VPS**, vía `.github/workflows/deploy-vps.yml` en push a `release`. La rama `release` solo avanza con fast-forward desde `main` y solo cuando el usuario lo pide; nunca se desarrolla en ella. El workflow construye la imagen en el runner con el `Dockerfile` de `app/`, la transfiere por SSH (`docker save | docker load`, sin registro de por medio), escribe `/srv/openrouter/.env` desde los secretos, ejecuta `vps/desplegar.sh <sha>` y verifica con `tools/verificar-servicio.sh`. A mano con un SHA anterior es un rollback. En el VPS: el Caddy que ya sirve otros dominios reenvía `apisor.oracle402.com` al contenedor publicado solo en `127.0.0.1:8081` (`vps/apisor.caddy` va a `/etc/caddy/sites.d/`, importado por nombre desde el `Caddyfile`); datos en `/srv/openrouter/datos`, copias diarias con `sqlite3 .backup` en `/srv/openrouter/copias` (`vps/copia.sh` + timer de systemd). `vps-preparar.yml` (a mano, idempotente) prepara la máquina; `vps-inspeccionar.yml` (a mano, solo lectura) dice qué hay en ella. Guía de operación en `docs/planificacion/runbook-vps.md`.
- Backend de previsualización (opcional): Fly.io vía `.github/workflows/deploy.yml`. `FLY_API_TOKEN` **llega heredado de la organización `npiobject-labs`** (secreto de organización, repos públicos); no hay que crear ni guardar ningún token por proyecto. Nunca lo imprimas en los logs.
