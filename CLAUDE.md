...
- El contrato del servicio vive bajo `/v1/`, compatible con la API de OpenAI donde hay equivalente. Publicada una ruta, no se rompe: lo incompatible iría a `/v2/`. Hoy existen `GET /v1/estado` (consulta la clave en OpenRouter, no gasta crédito), `GET /v1/models` (catálogo con caché de una hora), `POST /v1/chat/completions` (proxy fino, con `stream: true` desde la etapa 7) las de uso (`GET /v1/uso`, `GET /v1/uso/{id}`, `GET /v1/uso/resumen` y `GET /v1/uso/exportar`) las de aplicaciones (`POST /v1/apps`, `GET /v1/apps` y `DELETE /v1/apps/{id}`) las de presupuesto (`GET /v1/presupuesto` y `PUT /v1/apps/{id}/presupuesto`) y las de alias (`GET /v1/alias`, `PUT /v1/alias/{nombre}`, `DELETE /v1/alias/{nombre}` y `GET /v1/alias/{nombre}/simular`).
...
- **Un alias es un nombre de trabajo, no un modelo** (`app/src/alias.rs`): una aplicación pide `"model": "alias:mapeador"` y el servicio decide el modelo, los parámetros por defecto y la cadena de respaldo. Los parámetros del alias **no pisan** lo que mande quien llama: el alias pone el valor por defecto, la consulta tiene la última palabra, y nunca puede fijar `model`, `messages`, `stream` ni `stream_options`, que son de la llamada y no del modelo. Se recurre al respaldo solo cuando el fallo es del proveedor o del modelo (`429`, `404`, `408`, `5xx`); un `400` fallaría igual en todos y reintentarlo solo gasta tiempo. En flujo el respaldo solo cabe antes de abrir el flujo: salida la primera cabecera, cambiar de modelo sería mentirle al cliente. Verlos es de cualquier clave, tocarlos es de administración, y un alias **se borra de verdad** (al revés que una aplicación) porque el histórico guarda su nombre como texto: `agrupar=alias` reparte el gasto incluso de los ya borrados. `GET /v1/alias/{nombre}/simular` le pone precio a la cadena entera sin llamar a nadie.
- **El streaming se mide de pasada, sin tocar los eventos** (`app/src/flujo.rs`): el cuerpo se reenvía tal cual, que es lo que entiende un cliente de la API de OpenAI, y un medidor mira los eventos al vuelo. El servicio fuerza `stream_options.include_usage` para que el último evento traiga el `usage`; sin eso, el coste de un flujo solo se sabría por la reconciliación. **El registro se escribe en `Drop`, no al terminar el bucle**: si el cliente corta a mitad, axum suelta el cuerpo y eso es lo único que se ejecuta, así que una respuesta cancelada queda anotada con `motivo_fin: "cancelado"` y lo que costó hasta ahí. `primer_token_ms` mide **desde que entra la petición**, no desde que vuelven las cabeceras de OpenRouter: ese tramo es casi todo lo que tarda el primer token, y dejarlo fuera daba ceros. El reloj lo pone `chat`, uno solo para los dos caminos, y el medidor lo recibe hecho. `Registro::desde_respuesta` no machaca con vacío lo que ya tiene, porque en flujo se aplica evento a evento y el del `usage` viene sin `id` ni `model`. `deploy.yml` lo verifica pidiendo ocho tokens en flujo, único paso que gasta crédito.
